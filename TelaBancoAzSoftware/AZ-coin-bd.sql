...
-- ----------- ou a mesma tabela acima, mas já com as alterações solicitadas (dispensa o procedimento de alteração da tabela que estão no final deste documento. 

As alterações estão destacadas em amarelo. 

Caso não seja executado este script para a criação desta tabela, deve-se executar as instruções  contidas no final deste documento. 
...
------------------------------- ALTERÇÕES APÓS A CRIAÇÃO DO BANCO DE DADOS ----------------------- 
...
ADD COLLUMN recuperar_senha, VARCHAR(255); 

 

ALTER TABLE usuário 

ALTER COLLUMN senha type varchar(250); 
...
Adicionado coluna recuperar_senha (varchar 255) na tabela usuário. 

Alterado tamanho da coluna senha para 250 caracteres (devido a criptografia). 


INSERT INTO perfil_usuario (descricao_perfil) values ('Administrador');
INSERT INTO perfil_usuario (descricao_perfil) values ('Gestor');
INSERT INTO perfil_usuario (descricao_perfil) values ('Colaborador');
